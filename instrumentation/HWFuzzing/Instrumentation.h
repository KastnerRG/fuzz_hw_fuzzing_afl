...
static unsigned getMapElementByteSize() {
  if (MapElementByteSize == 0) {
    // Mirrors the code in afl-compiler-rt.o.c.
...
[[noreturn]] inline void exitWithErr(std::string error) {
  llvm::errs() << "fatal error: " << error << "\n";
  abort();
}

[[noreturn]] inline void exitWithErr(std::string error, llvm::Value *v) {
  llvm::errs() << "fatal error: " << error << " - ";
  if (v)
    llvm::errs() << *v;
  else
...
struct HardwareInstrumentation {
  ...
  void addToCoverageMap(IRBuilder<> &IRB, MapElementOffset mapOffset,
    ...
    if (mapOffset < minAllowedMapAccess)
      exitWithErr("mapOffset below allowed range (" +
                  std::to_string(mapOffset) + " < " +
                  std::to_string(minAllowedMapAccess) + ")");
    if (mapOffset >= maxAllowedMapAccess)
      exitWithErr("mapOffset beyond allowed range (" +
                  std::to_string(mapOffset) +
    ...
    if (coverage == nullptr) exitWithErr("coverage is a nullptr?");
  ...
  // (see below).
  enum class IsPartOfConditionVec { Yes, No };

  void addConditionToCoverageMap(
      IRBuilder<> &IRB, MapElementOffset mapOffset, Value *condition_raw,
  ...
  void doSelectFeedback(llvm::SelectInst &i, MapElementOffset mapOffset) {
    ...
    for (unsigned i = 0; i < t->getNumElements(); ++i) {
      ...
      addConditionToCoverageMap(
          IRB, mapOffset + i, condition_bit,
          i == 0 ? IsPartOfConditionVec::No : IsPartOfConditionVec::Yes);
    }
  }
  ...
  void doToggleFeedback(llvm::StoreInst &i, unsigned long mapOffset) {
    ...
    if (!canDoToggleFeedback(i)) exitWithErr("Bogus toggle feedback inst?", i);
  ...
  unsigned findCoveragePoints(Function &F) {
    ...
    for (auto &BB : F) {
      ...
      for (auto &I : BB) {
        ...
        if (I.hasMetadata(I.getModule()->getMDKindID("nosanitize"))) continue;
        ...
        if (isModeOn(CoverageMode::Edge) ||
            isModeOn(CoverageMode::BasicBlock)) {
  ...
  void injectCoverage() {
    ...
    for (const auto &target : toInstrument) {
      ...
      else { exitWithErr("Not a valid coverage point?"); }
    }
  }
};
